...
namespace 帮我查
{
    ...
    /// </summary>
    public sealed partial class SUBmain_shenfenzheng : Page
    {
        private NavigationHelper navigationHelper;
        private ObservableDictionary defaultViewModel = new ObservableDictionary();

        public SUBmain_shenfenzheng()
        {
        ...
        private async void _htmltianqiV_DOMContentLoaded(WebView sender, WebViewDOMContentLoadedEventArgs args)
        {
            try
            {
                ...
                //自动调用脚本启动查询
                if (_htmltianqiV.Tag != null && _htmltianqiV.Tag.ToString().IndexOf("CS:") >= 0)
                {
                    string sfz = _htmltianqiV.Tag.ToString().Replace("CS:", "");
                    _htmltianqiV.Tag = null;
                    // 向 html 中注册脚本,自动进行查询
                    List<string> arguments2 = new List<string> { "function ReSet_yhb_zidongchaxunshenfenzheng(){ $(\"#userid\").attr(\"value\",'" + sfz + "'); searchCard();}" };
                    await _htmltianqiV.InvokeScriptAsync("eval", arguments2);

                    // 调用刚刚注册的脚本
                    await _htmltianqiV.InvokeScriptAsync("ReSet_yhb_zidongchaxunshenfenzheng", null);
                    
                }
          
          
                // 向 html 中注册脚本,删除不想让用户看到的东西
                List<string> arguments = new List<string> { "var ReSet_yhb_haha_num = 0; var ReSet_yhb_haha_interval = null; function ReSet_yhb_haha(){ if(ReSet_yhb_haha_num > 20){clearInterval(ReSet_yhb_haha_interval)};   $(\"header\").hide(); $(\"footer\").hide(); $(\".navfixed\").hide(); return 'ok';}  function ReSet_yhb_haha_dingshi(){window.setInterval(ReSet_yhb_haha, 100); }" };
        ...
        private void citie_Click(object sender, RoutedEventArgs e)
        {
            ...
            Pin("ms-appx:///Assets/Square150x150Logo.scale-240.png", this.GetType().Name, "帮我查-身份证");
        }
    }
}
